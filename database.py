...
if st.session_state.get("login", False):
    ...
    if menu == "Lihat Data":
        ...
        df = pd.DataFrame([(m.id, m.kode_mesin, m.nama_mesin, m.brand, m.sg, m.nomor_dokumen,
                            ...
                            m.tanggal_sewa, m.jumlah_sewa or 0, m.tanggal_dikembalikan, m.jumlah_mesin_kembali or 0,
    ...
    # ---------------------------
        ...
        if st.session_state["role"] != "admin":
            ...
            tanggal_penjualan = st.date_input("Tanggal Penjualan", datetime.now())
            tanggal_sewa = st.date_input("Tanggal Sewa", datetime.now())
            jumlah_sewa = st.number_input("Jumlah Sewa", min_value=0, value=0)
            tanggal_dikembalikan = st.date_input("Tanggal Dikembalikan", datetime.now())
                ...
                try:
                    ...
                    if exist:
                        ...
                        m = Mesin(
                            ...
                            tanggal_penjualan=datetime.combine(tanggal_penjualan, datetime.min.time()),
                            tanggal_sewa=datetime.combine(tanggal_sewa, datetime.min.time()),
                            jumlah_sewa=jumlah_sewa,
                            tanggal_dikembalikan=datetime.combine(tanggal_dikembalikan, datetime.min.time()),
    ...
    # ---------------------------
        ...
        if st.session_state["role"] != "admin":
            ...
            mesin = session.get(Mesin, pilih_id)
